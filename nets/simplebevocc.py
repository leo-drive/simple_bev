...
class Decoder(nn.Module):
    def __init__(self, in_channels, n_classes, out_dim=32):
        super().__init__()
        self.out_dim=out_dim
        self.decoder = nn.Sequential(
            nn.Conv3d(in_channels, out_dim, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm3d(out_dim),
            nn.Conv3d(out_dim, out_dim, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm3d(out_dim)
...
class SimpleBEVOCC(nn.Module):
    def __init__(self, Z, Y, X, vox_util=None, 
                 ...
                 latent_dim=128,
                 encoder_type="res101"):
        super(SimpleBEVOCC, self).__init__()
